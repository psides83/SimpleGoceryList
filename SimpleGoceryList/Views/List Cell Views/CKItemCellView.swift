...
struct CKItemCellView: View {
    ...
    @State private var isPicked: Bool
...
//    @State private var image: Data
    ...
    init(item: CKItem, list: CKList, editMode: Binding<EditMode>) {
        self.item = item
        self.list = list
        
        _name = State(wrappedValue: item.name ?? "")
        self._editMode = editMode
        _quantity = State(wrappedValue: item.quantity)
        _isPicked = State(wrappedValue: item.isPicked)
    }
...
//            update(quantity)
    ...
    var statusCircle: some View {
        if editModeInactive {
            Toggle("", isOn: $isPicked)
                .toggleStyle(CheckToggleStyle())
                .onChange(of: isPicked, perform: { value in
                    pickToggle()
                })
...
//        Text("\(modifiedText) \(item.lastModified)")
//            .font(.caption2)
//            .foregroundColor(.picked)
//    }
...
//    private func update(_ quantity: Int64) {
//        withAnimation {
//
...
//            item.isPicked.toggle()
            sharedItemRepo.addItem(name: name, timestamp: item.timestamp, isPicked: isPicked, isUrgent: item.isUrgent, lastModified: Date(), order: item.order, quantity: item.quantity) { _ in
                isAddingList = false
            }
...
//    private func delete(_ item: CKItem) {
//        withAnimation {
