            ...
            NavigationView {
                VStack(spacing: 0) {
                    List(selection: $selection) {
                        ...
                        if !nameIsFocused && list.pickedItems.count != 0 {
                    ...
                    .navigationBarTitle(list.titleDisplay)
                    ...
                    HStack {
                        Button(action: {
                            sharedItemRepo.addList(title: list.titleDisplay, timestamp: list.timestamp, list: list) { _ in
                                isAddingList = false
                            }
...
//            }
    ...
    var unpickedSection: some View {
        Section(header:
            ...
            ForEach(list.unpickedItems, id: \.self) { item in
                ZStack {
                    
                    ListItemCellView(item: item, editMode: $editMode)
                }
            }
            ...
            .onDelete { offsets in
                ...
                let allItems = list.unpickedItems
    ...
    var pickedSection: some View {
        Section(header:
            ...
            ForEach(list.pickedItems, id: \.self) { item in
                
                ListItemCellView(item: item, editMode: $editMode)
            }
            .onDelete { offsets in
                let allItems = list.pickedItems
    ...
    var removeUnpickedButton: some View {
        if editMode == .active && list.unpickedItems.count != 0 && selection.filter({!$0.isPicked}).count != 0 {
            Button {
                withAnimation {
    ...
    var textFieldResults: [Item] {
        ...
        list.listItems.forEach { listItem in
            suggestedItems.removeAll(where: {$0.nameDisplay.contains(listItem.nameDisplay)})
        }
    ...
    var nonDuplicate: [Array<Any>] {
        [list.listItems.filter({$0.nameDisplay != nameText}), items.filter({$0.nameDisplay != nameText})]
    }
    
    var duplicateInUse: [ListItem] {
        list.listItems.filter({$0.nameDisplay == nameText})
    }
    ...
    private func addItem() {
        ...
        if canCreate {
            withAnimation {
                ...
                newListItem.order = Int16(list.unpickedItems.count + 1)
...
//                }
    ...
    private func removeFromList(_ item: ListItem) {
        withAnimation {
            viewContext.perform {
...
//                persistenceController.update(newItem)
                sharedItemRepo.addItem(name: nameText, timestamp: Date(), isPicked: false, isUrgent: false, lastModified: Date(), order: 1, quantity: 1) { _ in
                        isAddingList = true
                }
    ...
    private func move( from source: IndexSet, to destination: Int) {
        ...
        var revisedItems: [ ListItem ] = list.unpickedItems.map{ $0 }
